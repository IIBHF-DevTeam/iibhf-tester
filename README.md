...
``IIBHFTester.New(TestSuiteName: string, RunOutsideOfStudio: boolean, OnTestEndWithFail: () -> (), OnTestEndWithoutFail: () -> ()): any`` - Makes a new test suite, it will return either an ``IIBHFClientTester`` or ``IIBHFServerTester`` on client and server respectfully, ``OnTestEndWithFail`` and ``OnTestEndWithout`` will run depending on if there were tests that failed or not
